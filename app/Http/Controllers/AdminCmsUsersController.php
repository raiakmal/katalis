...
class AdminCmsUsersController extends CBController
{
    public function cbInit()
    {
        ...
        if(Cx::myPrivilegeId() == 5) {
            $this->form[] = ["label"=>"Nama Pemohon/Instansi/Perusahaan","name"=>"company", "type" => "text", 'validation'=>'required|string|min:3|max:50|unique:cms_users,name'];
            $this->form[] = ["label"=>"Nama Penghubung","name"=>"name", "type" => "text", 'validation'=>'required|string|min:3|max:50|unique:cms_users,name'];
            ...
            $this->form[] = ["label"=>"Email","name"=>"email",'type'=>'email','validation'=>'required|email|max:75|unique:cms_users,email', "exception" => true, "disabled" => true];
            $this->form[] = ["label"=>"No. WhatsApp","name"=>"phone",'type'=>'number','validation'=>'required|numeric|regex:/^08[0-9]{8,11}$/','help'=>'Dimulai dari 08XXXXXXXXXX, Contoh: 081612341234'];
           
            $this->form[] = ["label"=>"Provinsi","name"=>"id_cms_provinsis", "type" => "select", 'validation'=>'required|integer|exists:cms_provinsis,id', "datatable" => "cms_provinsis,name"];
            $this->form[] = ["label"=>"Kota/Kabupaten","name"=>"id_cms_kabupatens", "type" => "select", 'validation'=>'required|integer|exists:cms_kabupatens,id', "datatable" => "cms_kabupatens,name", "datatable_ajax" => 1, "parent_select" => "id_cms_provinsis"];
        ...
        $this->form[] = ["label"=>"Password","name"=>"password","type"=>"password", "required" => request()->segment(3) == 'add' ? true : false, "help"=> request()->segment(3) <> 'add' ? "Kosongkan jika tidak ada perubahan" : ""];
        $this->form[] = ["label"=>"Password Confirmation","name"=>"password_confirmation", "required" => request()->segment(3) == 'add' ? true : false, "type"=>"password", "help"=>request()->segment(3) <> 'add' ? "Kosongkan jika tidak ada perubahan" : ""];
